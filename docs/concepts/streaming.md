...
- `progress` streams Slack's native agent card by default: one message carries
  narration, the live plan/task card, and the final answer. Routine progress
  updates coalesce at one-second intervals; completion flushes immediately. The
  card appears only for turns that do real work, so plain questions are
