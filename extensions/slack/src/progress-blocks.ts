...
// Native cards derive from the compositor snapshot: an authored plan owns the
// rows, otherwise the rolling tool lines do, and reconciliation retires rows
...
function buildPlanTasks(params: {
  ...
  return lines.map((line) => {
    const identity = resolveLineTaskIdentity(line);
    let id = identity.id;
...
function resolvePlanTitle(params: {
  ...
  const newest = params.tasks.at(-1);
...
export function buildSlackProgressStreamChunks(params: {
  ...
  if (tasks.length === 0) {
    ...
    if (!params.sessionUrl && !diffOutput) {
...
function buildActivityText(lines: readonly ChannelProgressDraftLine[], maxLineChars: number) {
  ...
  for (const line of lines.slice(-SLACK_MAX_BLOCKS).toReversed()) {
    const title = `${line.icon ?? "•"} *${escapeSlackMrkdwn(line.label)}*`;
    const row = `${title} — ${activityLineDetail(line, maxLineChars)}`;
...
export function buildSlackProgressCardBlocks(params: {
  ...
  const sections = [
    `${icon} *${escapeSlackMrkdwn(params.title.trim() || "Working")}*`,
    narration ? `_${escapeSlackMrkdwn(narration)}_` : "",
    planLines.map((line) => escapeSlackMrkdwn(line)).join("\n"),
    buildActivityText(params.lines, maxLineChars),
  ];
